...
export const academyModules: AcademyLesson[] = [
  ...
  ...closeObservationLessons,
  {
    ...
    check: {
      ...
      choices: [
        ...
        { id: "sample", label: bi("薄薄的樣品能讓光穿過，這次想看清楚細胞邊界", "The thin specimen transmits light; we want cell boundaries"), feedback: bi("是的！樣品要合適，也需要照光和對焦，才能看清楚細胞，不只靠倍率。", "Yes! A suitable specimen, illumination and focus reveal cells, beyond just magnification.") },
      ],
    },
    icon: "microscope",
    ...
    clue: bi(
      "看見：對焦能讓細胞的細節變清楚；把圖片放得更大，不代表能看到新的細節。",
  ...
  },
  {
    ...
    clue: bi(
      "深入：看更細的構造，也要先準備合適的樣品。不是所有問題都適合用電子顯微鏡。",
...
export const toolAbilities = [
  ...
  },
  {
    ...
    description: bi("觀察更細小的結構", "Observe finer structures"),
  },
];
...
export const scaleSequence = [
  ...
  { label: bi("成體果蠅", "Adult fruit fly"), image: "fruit-fly" },
  { label: bi("細胞", "Cell"), image: "animal-cell" },
  { label: bi("細菌", "Bacterium"), image: "bacterium" },
  { label: bi("病毒", "Virus"), icon: "search" },
  { label: bi("更細小的細胞構造", "Finer cell structures"), icon: "search" },
];

export const academyVisuals = {
  scaleNote: bi(
    "這些例子大致從大排到小。卡片沒有照真正的大小比例畫，不同種類也會有大小差別。不用背數字！",
