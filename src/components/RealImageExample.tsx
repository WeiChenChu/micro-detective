...
export function RealImageExample({ illustrationId, locale }: { illustrationId: string; locale: Locale }) {
  ...
  return <details className="real-image-example" open>
    <summary>{real ? real.title[locale] : slot.title[locale]}</summary>
    {real && real.type === "real" ? <>
      <p>{locale === "zh-TW" ? "示意圖幫助我們理解；真實影像讓我們看看顯微鏡真正拍到了什麼。這是另一份樣品，不是剛才示意圖的連續放大。" : "Illustrations help us understand; real images show us what microscopes actually capture. This is a different specimen, not a continuous zoom."}</p>
      <MicroscopyImage id={real.id} locale={locale} showCaption />
      {real.observationClue && <p className="observation-clue">{real.observationClue[locale]}</p>}
