...
**v0.21**（npm 套件版本 `0.2.1`）— 適合國小高年級至國中及親子科普活動的靜態教育遊戲。
...
**課程：** 六個約 1–2 分鐘的模組，依序認識肉眼、放大鏡、解剖顯微鏡、光學、螢光、電子。放大鏡可拖曳、用方向按鈕或鍵盤操作；解剖顯微鏡以完整果蠅與局部表面說明視野與立體感。先觀察與操作，再做一題檢核，讀解說並收藏知識卡。螢光沿用四步演示，明確說明它是光學顯微鏡的一種。光學開燈與電子細節比較可用鍵盤操作。

**選工具小遊戲：** 四題重用既有作答與回饋元件；整體外形接受肉眼、放大鏡或解剖顯微鏡，翅膀接受放大鏡或解剖顯微鏡。依所選工具解說原因。完成技能可在筆記本複習，並繼續原本任務。

**任務：**

1. 誰需要顯微鏡？— 比較成魚、成果蠅、細胞與細菌。
2. 找出微觀線索 — 選擇能讓細胞核更明顯的標記方法。
3. 神秘影像 — 四份一般光學、螢光、SEM、TEM 示意，選出支持判斷的可見證據與觀察記錄。
...
- v0.21 延用 v0.2 鍵：`microscopic-detective:progress:v2`，schema 2、content 2、academyRevision 2。舊 v0.2 課程索引按 ID 遷移，保留任務與已收藏的四張卡；新增小遊戲獨立保存。只保存這台裝置的語言、任務作答、課程頁與收藏。
...
第一個可玩 MVP 以 annotated tag **`v0.1`** 保留；當時的套件版本為 **`0.1.0`**。本次工作目錄已更新為 **`0.2.1`**（顯示 v0.21），尚未新增版本 tag 或發布線上版本。
