...
**v0.26**（npm 套件版本 `0.26.0`；開發分支 `dev/v0.26`）— 適合國小三～六年級及親子科普活動的靜態教育遊戲。
...
六張知識卡新增專案內建工具 SVG、工具名稱與一句話用途；原有內容保留，延伸線索可展開。圖片與摘要由 `src/data/toolIllustrations.ts` 管理，資產位於 `src/assets/tools/`。完成六堂課後可優先翻閱筆記本，也可直接開始任務；首次工具選擇題提供一次性筆記本提示。維持「課程 → 知識卡 → 偵探筆記本」架構。
...
- v0.26 延用 v0.2 鍵：`microscopic-detective:progress:v2`，schema 2、content 3、academyRevision 2。本次保留 v0.22／v0.23 的任務、練習、課程 ID、正解與儲存格式；v0.22／v0.23 存檔在原有 24 小時效期內可直接續玩。較舊 v0.2／v0.21 的內容版本仍依既有規則重置。只保存這台裝置的語言、任務作答、課程頁、收藏與一次性提示狀態。有效 v0.25 存檔缺少新提示欄位時也可續玩。
...
第一個可玩 MVP 以 annotated tag **`v0.1`** 保留；當時的套件版本為 **`0.1.0`**。本次工作目錄已更新為 **`0.26.0`**（顯示 v0.26），尚未新增版本 tag 或發布線上版本。
...
本次結果見 [v0.26 驗證紀錄](docs/v0.26-validation.md)；前版記錄保留於 [v0.25 驗證紀錄](docs/v0.25-validation.md)。本次沒有下載或發布真實顯微影像。既有 `source_images/bioart/` 為儀器插圖，`image_credits/IMAGE_CREDIT.md` 為使用者提供的候選素材紀錄，尚不等於網站已載入或已逐項驗證。
