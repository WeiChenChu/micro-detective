...
**v0.22**（npm 套件版本 `0.2.2`）— 適合國小高年級至國中及親子科普活動的靜態教育遊戲。
...
**課程：** 六個約 1–2 分鐘的模組，依序認識肉眼、放大鏡、解剖顯微鏡、複式光學顯微鏡、螢光、電子。放大鏡可拖曳、用方向按鈕或鍵盤操作；解剖顯微鏡以完整果蠅與局部表面說明視野與立體感。先觀察與操作，再做一題檢核，讀解說並收藏知識卡。螢光沿用四步演示，明確說明它是光學顯微鏡的一種。複式光學顯微鏡開燈與電子細節比較可用鍵盤操作。
...
src/data/
  academyData.ts     六個課程、檢核、知識卡、尺度與三種能力
...
| 課程、檢核、知識卡與概念說明 | `src/data/academyData.ts` |
...
- v0.22 延用 v0.2 鍵：`microscopic-detective:progress:v2`，schema 2、content 3、academyRevision 2。本次練習正解與問題改變，依既有內容版本驗證，舊 v0.2／v0.21 進度（含任務、課程收藏與練習）會重置；v0.22 進度可正常續玩。只保存這台裝置的語言、任務作答、課程頁與收藏。
...
- 重新整理恢復任務作答、提示及課程頁／已收藏卡片。未完成課程的演示與檢核會從該模組觀察頁重開，不影響已收藏卡片。
...
第一個可玩 MVP 以 annotated tag **`v0.1`** 保留；當時的套件版本為 **`0.1.0`**。本次工作目錄已更新為 **`0.2.2`**（顯示 v0.22），尚未新增版本 tag 或發布線上版本。
